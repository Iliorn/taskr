...
func TestScriptAddLearning(t *testing.T) {
	task := todo.New("Learned something")
	m := modelWithTasks(t, task)

	m = openDetailOn(t, m, fieldLearnings)
	m = sendKey(t, m, "a")
	if m.mode != modeAddLearning {
		t.Fatalf("after 'a': mode = %v, want modeAddLearning", m.mode)
	}
	m = script(t, m, "always check the fuse first", "enter")
	assertBackToNormal(t, m, "adding a learning")

	got := m.get(task.ID)
	if len(got.Learnings) != 1 {
		t.Fatalf("got %d learnings, want 1", len(got.Learnings))
	}
	if got.Learnings[0].Text != "always check the fuse first" {
		t.Errorf("learning = %q", got.Learnings[0].Text)
	}
	if !m.savePending && !m.saveScheduled {
		t.Error("adding a learning did not schedule a save")
	}

	m = sendKey(t, m, "u")
	if len(m.get(task.ID).Learnings) != 0 {
		t.Error("undo did not remove the added learning")
	}
}

func TestScriptAddLearningRejectsBlank(t *testing.T) {
	task := todo.New("No empty learnings")
	m := modelWithTasks(t, task)

	m = openDetailOn(t, m, fieldLearnings)
	// An empty learning renders as a blank bullet that can never be found
	// by `taskr learnings` — it is a row with nothing in it.
	m = script(t, m, "a", "   ", "enter")
	if len(m.get(task.ID).Learnings) != 0 {
		t.Errorf("a whitespace-only learning was stored: %+v", m.get(task.ID).Learnings)
	}
}

func TestScriptEditLearning(t *testing.T) {
	task := todo.New("Refine a learning")
	task.AddLearning("half an idea")
	m := modelWithTasks(t, task)

	m = openDetailOn(t, m, fieldLearnings)
	m = sendKey(t, m, "enter")
	if m.mode != modeEditLearning {
		t.Fatalf("after 'enter': mode = %v, want modeEditLearning", m.mode)
	}
	if got := m.textInput.Value(); got != "half an idea" {
		t.Fatalf("input = %q, want the existing learning", got)
	}
	m = script(t, m, " refined", "enter")
	assertBackToNormal(t, m, "editing a learning")

	got := m.get(task.ID)
	if len(got.Learnings) != 1 {
		t.Fatalf("got %d learnings, want 1 — editing must not append", len(got.Learnings))
	}
	if got.Learnings[0].Text != "half an idea refined" {
		t.Errorf("learning = %q", got.Learnings[0].Text)
	}
}

func TestScriptDeleteLearning(t *testing.T) {
	task := todo.New("Drop a learning")
	task.AddLearning("wrong conclusion")
	m := modelWithTasks(t, task)
	...
	// The prompt quotes the learning, so the user can tell which row they
	// are about to lose.
	if !strings.Contains(m.confirmMsg, "wrong conclusion") {
		t.Errorf("confirm message %q should name the learning", m.confirmMsg)
	}
	m = sendKey(t, m, "y")
	assertBackToNormal(t, m, "confirming a learning delete")
	if len(m.get(task.ID).Learnings) != 0 {
		t.Error("the learning was not deleted")
	}
	m = sendKey(t, m, "u")
	if len(m.get(task.ID).Learnings) != 1 {
		t.Error("undo did not restore the learning")
	}
}
