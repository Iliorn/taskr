...
func mergeIntoStoreOnce(h *sql.DB, incoming []todo.Todo) ([]todo.Todo, bool, error) {
	...
	// No-op guard: if the merge changed nothing, don't write — the rollback in
	// the deferred call releases the read snapshot without churning the DB.
	if storeDigest(current) == storeDigest(merged) {
	...
	if err := saveNormalizedIn(tx, ptrs, nil); err != nil {
		return nil, false, err
	}
