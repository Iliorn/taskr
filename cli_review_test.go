...
func TestSortTodosByCLIModeOrdersAndRejectsUnknown(t *testing.T) {
	...
	if err := sortTodosByCLIMode(rows, "age", nil); err != nil {
	...
	rows = append([]todo.Todo(nil), base...)
	if err := sortTodosByCLIMode(rows, "idle", nil); err != nil {
	...
	if err := sortTodosByCLIMode(rows, "pri", nil); err != nil {
	...
	if err := sortTodosByCLIMode(rows, "colour", nil); err == nil {
...
func TestCLISortsAreTotalOrders(t *testing.T) {
	...
	for _, mode := range cliSortNames() {
		...
		if err := sortTodosByCLIMode(first, mode, nil); err != nil {
			t.Fatalf("sort %s: %v", mode, err)
		}
		if err := sortTodosByCLIMode(second, mode, nil); err != nil {
			t.Fatalf("sort %s: %v", mode, err)
		}
		for i := range first {
