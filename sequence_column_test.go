...
func assertSequenceColumn(t *testing.T, h *sql.DB, rk ranker, taskID string, live *todo.Todo, label string) {
	t.Helper()
	var col float64
...
func TestSequenceColumnTracksFormulaAfterSave(t *testing.T) {
	h := openTestDB(t)
	balanced := ranker{biases: biases{Deadline: biasBalanced, Priority: biasBalanced, Momentum: biasBalanced, Aging: true}}
...
// TestSequenceColumnDriftsWithoutResync documents the bug: when the biases
...
func TestSequenceColumnDriftsWithoutResync(t *testing.T) {
	...
	intense := ranker{biases: biases{Deadline: biasIntense, Priority: biasIntense, Momentum: biasIntense, Aging: true}}
